...
# ---- Configuration ----
# Model file expected to be present in the same folder of the repo when deployed
LOCAL_MODEL_FILENAME = "final_model_noopt.keras"
# Raw GitHub fallback (same file in your repo)
...
TOP_K_DEFAULT = 3
TILE_SIZE = 224             # crop size (your working setting)
BATCH_SIZE = 64             # safe default; adjust if needed
MAX_TILE_THUMBNAILS = 40    # limit thumbnails shown to keep UI snappy
...
def get_model_from_repo(local_name: str = LOCAL_MODEL_FILENAME, raw_url: str = GITHUB_RAW_MODEL_URL):
    """
    Prefer local model file from the repo. If missing, fetch from GitHub raw URL
    into /tmp/model.keras and load it.
    """
    # 1) Try local (repo) path
    ...
    # 2) Fallback: download from GitHub raw to /tmp
...
# ---- Sidebar (cleaned: no Drive controls) ----
with st.sidebar:
    st.header("Prediction Settings")
    top_k = st.number_input("Top K predictions (for single-image mode)", min_value=1, max_value=10, value=TOP_K_DEFAULT)
    use_vgg = st.checkbox("Use VGG preprocessing (/255.0 off)", value=USE_VGG_PREPROCESS)
    st.caption("Model is loaded from the repository file `final_model_noopt.keras` "
               "or fetched from GitHub raw if not present locally.")

# ---- Load model from repo / raw GitHub ----
    ...
    if stitched_image is not None:
        ...
        try:
            ...
            if len(boxes) == 0:
                ...
                # Deterministic mapping of labels to colors
                label_to_color = {}
...
v4 04nov2025 (GitHub model loader)
"""
)
